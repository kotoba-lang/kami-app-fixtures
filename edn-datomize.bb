...
;;                                                       (既に名前空間付きのキーはそのまま保持し、
;;                                                        裸キーだけ ns を付与。com-etzhayyim-futawa の
...
;;   bb edn-datomize.bb catalog-vector <path> <top-key> — {top-key [{item}...]} 形式の
;;                                                       カタログファイルを multi-entity
;;                                                       tx-data に変換（各 item は既に
;;                                                       慣用的に名前空間付きキーを持つ前提。
;;                                                       wrap-key を剥がし item ごとに
;;                                                       :db/id -1,-2,... を付与）。
;;
;; このコピーは kotoba-lang/kami-app-fixtures 用に schema-path のみ調整（manifest/ が
;; 無いのでリポジトリルート直下 schema.edn）＋ catalog-vector モードを追加。他のロジックは
...
;; ---------- catalog-vector ({top-key [{item}...]} → multi-entity tx-data) ----------
;;
;; kami-app-fixtures の resources/kami/app_fixtures.edn は
;; {:kami/app-fixtures [{:kami.fixture/id :survivors ...} ...]} という形—
;; トップレベルは単一キーで、値がすでに idiomatic に名前空間付きの item map の
;; ベクタ。wrap-map!/wrap-map-keep-ns! は「map 1個 → entity 1個」専用なので、
;; ここでは top-key を剥がし item ごとに :db/id を振って multi-entity tx-data に
;; する（item のキーはすでに :kami.fixture/* 等の名前空間付きなのでそのまま保持）。

(defn wrap-catalog-vector! [rel-path top-key-str]
  (let [f (io/file root rel-path)
        content (slurp-edn f)
        top-key (keyword top-key-str)]
    (if (already-tx-data? content)
      (println "skip (already tx-data):" rel-path)
      (let [items (get content top-key)
            entities (map-indexed
                      (fn [i item]
                        (into {:db/id (- (inc i))}
                              ...
                              item))
                      items)
            attrs (mapcat (fn [item] (for [[k v] item]
                                        (let [{:keys [type card]} (classify v)]
                                          {:db/ident k :db/valueType type :db/cardinality card})))
                          items)]
        (spit f (pr-str (vec entities)))
        (merge-schema! attrs)
        (println "wrapped(catalog-vector)" rel-path "->" (count entities) "entities")))))
...
(defn -main [& args]
  (let [[mode a b] args]
    (case mode
      ...
      "catalog-vector"   (wrap-catalog-vector! a b)
      "adr-dir"  (adr-dir! a)
      "adr-file" (let [report (atom {:ok [] :skipped [] :errors [] :attrs []})]
                   (adr-file! (io/file root a) report)
                   (merge-schema! (:attrs @report))
                   (println @report))
      (do (println "usage: bb edn-datomize.bb [wrap-map <path> <ns> | wrap-map-keep-ns <path> <ns> | catalog-vector <path> <top-key> | adr-dir <dir> | adr-file <path>]")
          (System/exit 1)))))
